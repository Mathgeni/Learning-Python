...
async def get_in(name, job, speed):
...
async def main():
    ...
    tasks = [
        asyncio.create_task(get_in(passenger['Name'], passenger['Job'], passenger['Speed'])) for passenger in passengers
    ]
    done, pending = await asyncio.wait(tasks, timeout=5)
    for task in done:
        print(f'{task.result()[0]} сел в автобус.')
    late = await asyncio.gather(*pending)
    print(*list(
        map(lambda task: f'{task[0]} {task[1]} не успел/а вовремя сесть в автобус.', late)), sep='\n')
